...
def gerar_matriz_de_confusao(resultados: List[Tuple[str, str]]):
    ...
    vp = df[(df['LLM forneceu resposta'] == LLM_RESPONDEU_SIM) & (df['Resposta é util?'] == AVALIACAO_SIM)].shape[0]

    # VN (Verdadeiro Negativo): LLM respondeu NÃO e a avaliação foi NÃO_CONSTA_NOS_DOCS
    vn = df[(df['LLM forneceu resposta'] == LLM_RESPONDEU_NAO) & (df['Resposta é util?'] == AVALIACAO_NAO_CONSTA)].shape[0]

    # FP (Falso Positivo): Agrupa Alucinações e Respostas Erradas
    fp_alucinacao = df[(df['LLM forneceu resposta'] == LLM_RESPONDEU_SIM) & (df['Resposta é util?'] == AVALIACAO_NAO_CONSTA)].shape[0]
    fp_resposta_errada = df[(df['LLM forneceu resposta'] == LLM_RESPONDEU_SIM) & (df['Resposta é util?'] == AVALIACAO_NAO)].shape[0]
    ...
    fn = df[(df['LLM forneceu resposta'] == LLM_RESPONDEU_NAO) & (df['Resposta é util?'] == AVALIACAO_NAO)].shape[0]
    ...
    print(f"Falsos Positivos (FP - Alucinações + Respostas Erradas): {fp} ({fp_alucinacao} aluc. + {fp_resposta_errada} err.)")
